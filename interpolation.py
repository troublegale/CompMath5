...
def newton_finite_difference_polynomial(xs, ys, n):
    ...
    return lambda x: ys[0] + sum([
        reduce(lambda a, b: a * b, [(x - xs[0]) / h - j for j in range(k)]) * delta_y[k, 0] / factorial(k) for k in
        range(1, n)
    ])
...
def solve(xs, ys, x, n):
    ...
    methods = [("Lagrange polynomial", lagrange_polynomial),
               ("Newton polynomial with divided differences", newton_divided_difference_polynomial),
               ("Newton polynomial with finite differences", newton_finite_difference_polynomial),
